...
const char * const BITCOIN_CONF_FILENAME = "crackedcoin.conf";
...
#else
    const char* pszModule = "crackedcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Crackedcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Crackedcoin";
#else
    // Unix
    return pathRet / ".crackedcoin";
#endif
#endif
}
